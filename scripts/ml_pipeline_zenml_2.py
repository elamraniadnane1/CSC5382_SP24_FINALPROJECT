...
def evaluate_model(model, data_loader, device):
    ...
    with torch.no_grad():
        try:
            ...
            # Ensure predictions are in the correct shape for classification report
            predictions = np.argmax(predictions, axis=1)

            report = classification_report(true_vals, predictions, output_dict=True)
            f1 = report['macro avg']['f1-score']
            cm = compute_confusion_matrix(true_vals, predictions)
            accuracy = compute_accuracy(true_vals, predictions)
            recall = compute_recall(true_vals, predictions)
            precision = compute_precision(true_vals, predictions)
...
def train_model(data: pd.DataFrame,train_data: pd.DataFrame, val_data: pd.DataFrame, hyperparams: dict) -> BertForSequenceClassification:
    ...
    def f1_score_func(preds, labels):
        # Ensure preds is not a string and is a numpy array or similars
        if isinstance(preds, str):
            raise ValueError(f"'preds' is a string, which is unexpected: {preds}")
    ...
    def evaluate(model, dataloader_val, device):
        ...
        # Wrap the evaluation process in a try-except block for robust error handling
        try:
            with torch.no_grad():
            ...
            report = classification_report(true_vals, np.argmax(predictions, axis=1), output_dict=True)
            f1 = report['macro avg']['f1-score']
    ...
    try:
        with mlflow.start_run():
            ...
            for epoch in tqdm(range(1, epochs+1), desc="Training Progress"):
                ...
                for batch in tqdm(dataloader_train, desc=f'Epoch {epoch}/{epochs}', leave=False):
                    model.zero_grad()
                    batch = tuple(b.to(device) for b in batch)
                ...
                loss_val_avg,cm,accuracy,precision,recall,f1,detailed_metrics, predictions, true_vals = evaluate_model(model, dataloader_val, device)
                val_f1 = f1_score_func(predictions, true_vals)
                loss_train_avg = loss_train_total/len(dataloader_train)
                # Progress bar for evaluation
                tqdm.write(f'Epoch {epoch}: Evaluating...')
                with tqdm(total=len(dataloader_val), desc="Validation Progress", leave=False) as pbar:
                    evaluation_metrics = evaluate_model(model, dataloader_val, device)
                    pbar.update(len(dataloader_val))

                if evaluation_metrics:
                    val_loss = evaluation_metrics['loss_val_avg']
                    tqdm.write(f'Epoch {epoch}/{epochs} - Training Loss: {loss_train_avg}, Validation Loss: {val_loss}, F1 Score: {evaluation_metrics["f1"]}')
                ...
                if evaluation_metrics:
                    ...
                    mlflow.log_metrics({
                        ...
                        "validation_f1": evaluation_metrics["f1"]
                    }, step=epoch)

                    for class_metric, scores in evaluation_metrics["detailed_metrics"].items():
                        mlflow.log_metrics({f'{class_metric}_precision': scores['precision'],
                                            f'{class_metric}_recall': scores['recall'],
                                            f'{class_metric}_f1-score': scores['f1-score']}, step=epoch)
                    # Calculate and display epoch metrics
                    loss_train_avg = loss_train_total/len(dataloader_train)
